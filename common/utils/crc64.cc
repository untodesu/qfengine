...
constexpr static const std::uint64_t CRC64_TABLE[256] = {
    /* clang-format off */ // <---- the table is too fat for clang-format to handle
    ...
    /* clang-format on */ // <---- the table is too fat for clang-format to handle
};
...
std::uint64_t utils::CRC64(const void* buffer, std::size_t size, std::uint64_t start)
{
    ...
    for(std::size_t i { 0 }; i < size; ++i) {
        start = CRC64_TABLE[((start >> 56) ^ data[i]) & 0xFF] ^ (start << 8);
    }
